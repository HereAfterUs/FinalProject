...
def getExperienceLineData(educational):
    hasEmpty = False
    if educational == '不限':
...
def getPeopleData():
    ...
    sorted_education_data = dict(sorted(educationData.items(), key=lambda item: item[1], reverse=False))

    return list(sorted_education_data.keys()), list(sorted_education_data.values())
